...
function reporter(context, options = {}) {
    ...
    const exceptPunctuation =
        options.exceptPunctuation !== undefined ? options.exceptPunctuation : defaultOptions.exceptPunctuation;
    ...
    const noSpaceBetween = (node, text) => {
        const betweenHanAndZen = matchCaptureGroupAll(
            text,
            /[A-Za-z0-9]([ 　])(?:[、。]|[\u3400-\u4DBF\u4E00-\u9FFF\uF900-\uFAFF]|[\uD840-\uD87F][\uDC00-\uDFFF]|[ぁ-んァ-ヶ])/
        );
        const betweenZenAndHan = matchCaptureGroupAll(
            text,
    ...
    const needSpaceBetween = (node, text) => {
        ...
        const reportMatch = (match) => {
            ...
            report(
                node,
                new RuleError("原則として、全角文字と半角文字の間にスペースを入れます。", {
                    index: match.index,
